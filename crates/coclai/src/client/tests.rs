...
use super::compat_guard::SemVerTriplet;
use super::config::DEFAULT_SCHEMA_RELATIVE_DIR;
use super::{
    ...
    validate_schema_dir, ClientConfig, ClientError, CompatibilityGuard, RunProfile, SessionConfig,
};
...
use crate::plugin_core_contract::{
    HookAction, HookContext, HookIssue, HookPhase, PostHook, PreHook,
};
...
impl PreHook for RecordingPreHook {
    ...
    fn call<'a>(
        &'a self,
        ctx: &'a HookContext,
    ) -> crate::plugin_core_contract::HookFuture<'a, Result<HookAction, HookIssue>> {
...
impl PostHook for RecordingPostHook {
    ...
    fn call<'a>(
        &'a self,
        ctx: &'a HookContext,
    ) -> crate::plugin_core_contract::HookFuture<'a, Result<(), HookIssue>> {
        let phases = Arc::clone(&self.phases);
        Box::pin(async move {
