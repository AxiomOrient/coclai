...
use crate::runtime::{RuntimeConfig, RuntimeHookConfig, SchemaGuardConfig, StdioProcessSpec};
...
struct PhasePatchPreHook {
    name: &'static str,
    patches: Vec<(HookPhase, crate::plugin::HookPatch)>,
}
...
async fn thread_start_rejects_privileged_sandbox_without_explicit_opt_in() {
    ...
    assert!(matches!(err, crate::runtime::errors::RpcError::InvalidRequest(_)));
