...
**Priority:** Low
**Scope:** `crates/codex-runtime/src/runtime/client/profile.rs`
...
**Priority:** Low
**Scope:** `crates/codex-runtime/src/runtime/rpc_contract.rs`
...
No new tests needed. The existing tests verify the output. After the change, confirm that `methods::KNOWN` is still accessible via `codex_runtime::runtime::rpc_contract::methods::KNOWN` (the public path in `rpc_contract::methods` must remain intact).
