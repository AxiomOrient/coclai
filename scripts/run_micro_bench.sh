...
if ! cargo run -q -p coclai --bin perf_micro_bench --release -- --help >/dev/null 2>&1; then
  echo "[micro-bench] skipped: coclai::perf_micro_bench binary is not available in this monolith revision"
  exit 0
fi

if [[ "$WRITE_BASELINE" == "1" ]]; then
  mkdir -p "$(dirname "$BASELINE_PATH")"
  cargo run -p coclai --bin perf_micro_bench --release -- \
    --out "$BASELINE_PATH" \
...
if [[ ! -f "$BASELINE_PATH" ]]; then
  cargo run -p coclai --bin perf_micro_bench --release -- "${ARGS[@]}"
...
attempt=1
while true; do
  if cargo run -p coclai --bin perf_micro_bench --release -- "${ARGS[@]}"; then
    if (( attempt > 1 )); then
